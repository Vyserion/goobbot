...
import { getLeaderboard, getLeaderboards, createLeaderboard, updateLeaderboard, deleteLeaderboard } from "./leaderboards";
...
describe("plugins/leaderboards/dao/leaderboards", () => {
    ...
    describe("createLeaderboards()", () => {
        ...
        it("should run the insert query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledWith(
                `INSERT INTO leaderboards VALUES (DEFAULT, $1, $2)`,
                [1, "test"]
            );
        });
    });
    ...
    describe("updateLeaderboard()", () => {
        ...
        it("should run the update query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `UPDATE leaderboards SET name = ($1) WHERE guild_id = $2 AND name LIKE ($3)`,
                ["newtest", 1, "test"]
            );
        });
    });
    ...
    describe("deleteLeaderboard()", () => {
        ...
        it("should run the delete query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `DELETE FROM leaderboards WHERE id = ($1)`,
                [1]
            );
        });
    });
