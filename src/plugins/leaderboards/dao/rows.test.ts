...
describe("plugins/leaderboards/dao/rows", () => {
    ...
    describe("createRow()", () => {
        ...
        it("should run the insert query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledWith(
                `INSERT INTO leaderboard_rows VALUES (DEFAULT, $1, $2)`,
                [1, "test row"]
            );
        });
    });
    ...
    describe("updateRowName()", () => {
        ...
        it("should run the update query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `UPDATE leaderboard_rows SET name = ($2) WHERE ID = ($1)`,
                [1, "new row name"]
            );
        });
    });
    ...
    describe("deleteRows()", () => {
        ...
        it("should run the delete query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `DELETE FROM leaderboard_rows WHERE leaderboard_id = ($1)`,
                [1]
            );
        });
    });
    ...
    describe("deleteRow()", () => {
        ...
        it("should run the delete query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `DELETE FROM leaderboard_rows WHERE leaderboard_row_id = ($1)`,
                [1]
            );
        });
    });
