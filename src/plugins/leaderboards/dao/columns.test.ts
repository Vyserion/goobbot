...
import { getColumn, getColumns, createColumn, updateColumnName, updateColumnType, deleteColumn, deleteColumns } from "./columns";
...
describe("plugins/leaderboards/dao/columns", () =>{
    ...
    describe("createColumns()", () => {
        ...
        it("should run the insert query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledWith(
                `INSERT INTO leaderboard_columns VALUES (DEFAULT, $1, $2, $3)`,
                [1, "test", ColumnTypes.DATA]
            );
        });
    });
    ...
    describe("deleteColumn()", () => {
        ...
        it("should run the delete query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `DELETE FROM leaderboard_columns WHERE leaderboard_id = ($1) AND id = ($2)`,
                [2, 1]
            );
        });
    });
    ...
    describe("deleteColumns()", () => {
        ...
        it("should run the delete query when called", async () => {
            ...
            expect(querySpy).toHaveBeenCalledTimes(1);
            expect(querySpy).toHaveBeenCalledWith(
                `DELETE FROM leaderboard_columns WHERE leaderboard_id = ($1)`,
                [1]
            );
        });
    });
