...
describe("plugins/lists/dao/values", () => {
	...
	describe("addValue()", () => {
		it("should run the insert query when called", async () => {
			...
			expect(querySpy).toHaveBeenCalledWith(`INSERT INTO list_value VALUES (DEFAULT, $1, $2)`, [1, "test"]);
		});
	});
	...
	describe("removeValue()", () => {
		it("should run the delete query when called", async () => {
			...
			expect(querySpy).toHaveBeenCalledTimes(1);
			expect(querySpy).toHaveBeenCalledWith(`DELETE FROM list_value WHERE list_id = $1 AND id = $2`, [1, 1]);
		});
	});
});
